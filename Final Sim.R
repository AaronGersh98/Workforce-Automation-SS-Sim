####################################################################
## BEFORE RUNNING CODE, IMPORT THE TWO DATASETS LISTED ON GITHUB ###
####################################################################
Init = as.matrix(Initial_Cond)
#agreggate automation probabilities
auto_prob = as.numeric(Init[,7])
set.seed(2)
C = 1
pr = 0
#Workforce Size
...
workforce.size = round(N*as.numeric(Init[,6]))
N = sum(workforce.size)
#assign a name to each worker based on their group
names.row = NA
for (i in 1:groups) {
...
skill.class = c("B","R")
#Creating blank matricies for future use
...
avg.income[,1] = as.numeric(Init[,5])
...
##INITIAL WAGES##
for (j in 1:N) {
  income[j,1] = abs(workforce.state[j,1]-1)*as.numeric(Init[which(Init == as.character(names.row[j])),5])
}
...
#Long-run equilibrium unemployment level for each job group
U.n = as.matrix(round(LRU*workforce.size + auto_prob*employed[,1]))
#Inititial unemployment level
unemployed[,1] = workforce.size - employed[,1]
#Linear interpolation as described in thesis
for (i in 1:groups) {
...
#Calculate number of employed workers each year
employed = workforce.size - unemployed
#Calculate number of jobs lost all years up to year n
auto = matrix(0,groups,n)
for (y in 2:n) {
  auto[,y] = round((unemployed[,y-1]/unemployed[,n-1])*as.matrix(auto_prob)*employed[,1])
}
#Calculate the number of jobs lost each year and the probability of any given job to be automated over the course of the year
for (i in 1:groups) {
...
#Calculate wage inequality over time
GINI.2 = rep(NA,y)
...
WBL.2 = rep(NA,years)
WBL.2[1] = 137700
#Index the WBL according the simulated wage increase rate
for (y in 2:years) {
  WBL.2[y] = WBL.2[y-1]*(1+g.2[y-1])
}
#Create a matrix showing the taxable base for each worker each month
...
#Aggregate tax base eac month
tot.tax = apply(tax.base, 2, sum)
tot.tax.m = apply(tax.base.m, 2, sum)
#Ratio of total wage base to total income to calculate ratio of taxable income
taxable.percent.2 = tot.tax/apply(tot.income, 2, sum)
plot(taxable.percent.2,type = "l")

#Apply time value of money to each month's tax base to calculate value of accumulated assets at time of retirement age
tyield = .025
...
#Create and index factor for wages
index = rep(NA, years)
index[years] = 1
z = 1+g.2
for (y in 1:(years-1)) {
...
#Index the incomes for each year
...
#Sort the indexed wages from largest to smallest and average the top 35
...
B1 = 960/index[1]
B2 = 5785/index[1]
#Apply PIA formula to previously calculate AIME
PIA.2 = matrix(NA,N,1)
...
EPV = rep(NA,N)
#Add yearly COLA value, but must ocnvert to monthly
...
tpx[1] = 1
ax = 0
#calculate ax using recursive formula
for (y in 1:max.life) {
  for (m in 1:12) {
...
taxrate.2
#This stuff is extra
#Calculate the value of the SS fund each year with newly found tax rate
tax.con = matrix(0,N,12*years+1)
for (m in 1:(12*years)) {
  tax.con[,m+1] = tax.con[,m]*(1+tyield)^(1/12)+(taxrate.2)*tax.base.m[,m]
}
#See what fraction of contributions came from those who aren't eligible
total.con = tax.con[,(12*years+1)]
uneligible.con.2 = sum(total.con[which(eligibility.2 == 0)])
uneligible.con.2/sum(total.con)
#See what the the tax rate would be if noneligible people got a refund on their contribution
